...
def test_ext_workspace_events_are_live_on_done_and_state() -> None:
    assert ext_workspace_events_are_live([(4, EXT_WS_MANAGER_DONE, b"")], 4)
    assert ext_workspace_events_are_live([(4, EXT_WS_MANAGER_WORKSPACE, struct.pack("<I", 1))], 4)
    assert ext_workspace_events_are_live([(9, EXT_WS_STATE, struct.pack("<I", 1))], 4)
    assert ext_workspace_events_are_live([(9, EXT_WS_REMOVED, b"")], 4)
    assert not ext_workspace_events_are_live([(9, EXT_WS_NAME, b"")], 4)
    assert not ext_workspace_events_are_live([(5, EXT_WS_MANAGER_DONE, b"")], 4)
    assert not ext_workspace_events_are_live([], 4)


def test_recv_wayland_available_drains_then_blocks() -> None:
    payload = pack_wayland_message(4, EXT_WS_MANAGER_DONE, b"")
    ...
    events, rest = recv_wayland_available(_Once(), b"")
    assert events == [(4, EXT_WS_MANAGER_DONE, b"")]
    assert rest == b""


def test_ext_workspace_live_watch_notifies_on_manager_done() -> None:
    payload = pack_wayland_message(4, EXT_WS_MANAGER_DONE, b"")
    hits: list[int] = []
...
def test_ext_workspace_live_watch_start_uses_scripted_display(monkeypatch: pytest.MonkeyPatch) -> None:
    from types import SimpleNamespace

    cancelled: list[int] = []

    class _Watchable(_ScriptedWorkspaceDisplay):
        def fileno(self) -> int:
            return 3
